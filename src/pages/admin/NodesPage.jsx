...
export default function NodesPage() {
  ...
  const loadNodes = async (nextPage = page) => {
    try {
      setIsLoading(true);
      setError(null);
      const response = await nodeService.list({ page: nextPage, limit: PAGE_SIZE });
      const pagination = extractPagination(response, 1);
      setNodes(extractList(response));
      setTotalPages(pagination.totalPages);
      setTotalNodes(pagination.total);
      setSelectedIds([]);
    } catch (err) {
    ...
    <>
      ...
      <main className="catalog-container">
        ...
        <p className="catalog-status" style={{ margin: "0 0 12px", padding: "0.8rem 1rem", fontSize: "0.95rem" }}>
          {isLoading ? "Loading nodes..." : `Showing ${nodes.length} nodes (total: ${totalNodes})`}
        </p>
